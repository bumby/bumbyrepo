...
for i in yytot:
    for j in mmtot:
     #   for target
        ...
        #scan 범위를 지정  
        #scan 범위는 현재 날짜에서 한달 전의 달의 옵션을 검색한다. 
        ...
        #기존 코스피 데이타의 일자를 모두 로드하고 시작 달/월의 index 를 확인한다. 
        #그리고 그달의 첫째날의 인덱스를 확인하고 현재지수로 부터 콜의 경우 +50 -30의 범위를
        # put의 경우 -50 +30의 범위의 
        for k in df["일자"]:
        ...
        a = np.arange(kospi_at_target-50, kospi_at_target+50,2.5)  #put
        print('targetmon_kospi_price',targetmon_kospi_price,'kospi_at_target',kospi_at_target, a)
     
        
        for k in a:
            for cnp in ["call","put"]:                                  #call과  put 모두 저장
                for codeindex in range(0,10):
                   ...
                   # optcode = dbanal.optcode_gen(k,i+j,"put")
                    optcode = dbanal.optcode_gen(k,i+j,cnp)
